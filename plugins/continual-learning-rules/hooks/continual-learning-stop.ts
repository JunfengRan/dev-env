...
const FOLLOWUP_MESSAGE =
  `Run the \`continual-learning\` skill now. Use incremental transcript processing with index file \`${INCREMENTAL_INDEX_PATH}\`: only read transcripts not in the index or transcripts whose mtime is newer than indexed mtime (re-read changed transcripts). After processing, write back the updated index mtimes and remove entries for deleted transcripts. Route each high-signal item into the appropriate \`.cursor/rules/*.mdc\` file (do NOT write to AGENTS.md). If no meaningful updates exist, respond exactly: No high-signal memory updates.`;
